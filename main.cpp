...
glm::vec4 Scene(const glm::vec3& Point)
{
	...
	Query = glm::vec4(0, 1, 1, Shapes::Floor(Point));
...
glm::vec3 CalcNormal(const glm::vec3& Point)
{
	...
	return glm::normalize(glm::vec3(
		Epsilon.xyy * Scene(Point + Epsilon.xyy).w +
		Epsilon.yyx * Scene(Point + Epsilon.yyx).w +
		Epsilon.yxy * Scene(Point + Epsilon.yxy).w +
		Epsilon.xxx * Scene(Point + Epsilon.xxx).w
	));
}
...
glm::vec4 March(const glm::vec3& Origin, const glm::vec3& Ray, bool* Hit)
{
	...
	{
		const std::float_t Precision = PREC * 0.001 * t.w;
		const glm::vec4 Query = Scene(Origin + Ray * t.w);
		if( Query.w < Precision )
		{
			*Hit = true;
			t = glm::vec4(Query.rgb, t.w);
			break;
		}
		if( t.w > FarPlane )
		{
...
std::float_t SoftShadow(const glm::vec3& LightPos, const glm::vec3& LightDir, std::float_t Min, std::float_t Max, std::float_t K)
{
	...
	{
		std::float_t Distance = Scene(LightPos + LightDir * glm::vec3(t)).w;
		if( Distance < PREC )
		{
			return 0.0f;
		}
...
void Scanline(std::size_t Start, std::size_t Quantum, RenderState& State)
{
	for( std::size_t Row = Start; Row < HEIGHT; Row += Quantum )
	{
		...
		for( std::size_t x = 0; x < WIDTH; x++ )
		{
			...
			glm::vec4 Query = March(State.EyePos, RayDir, &Hit);
